...
class Opt:
    def __init__(self):
        self.ckpt = "E:\\models\\model.ckpt"
        self.config = "E:\stable-diffusion-webui-master\configs\stable-diffusion\\v1-inference"
        self.defaults = "E:\stable-diffusion-webui-master\configs\webui/webui.yaml"
        self.esrgan_cpu = False
        self.esrgan_gpu = 0
        self.extra_models_cpu = False
        self.extra_models_gpu = False
        self.gfpgan_cpu = False
        self.gfpgan_dir = "E:\stable-diffusion-webui-master\src\gfpgan"
        self.gfpgan_gpu = 0
        self.gpu = 0
        self.grid_format = "jpg:95"
        self.inbrowser = False
        self.ldsr_dir = "E:\stable-diffusion-webui-master\src\latent-diffusion"
        self.n_rows = -1
        self.no_half = False
        self.no_progressbar_hiding = False
        self.no_verify_input = False
        self.optimized_turbo = True
        self.optimized = False
        self.outdir_img2img = None
        self.outdir_imglab = None
        self.outdir_txt2img = None
        self.outdir = "E:\\ai-server-eee\src\\temp"
        self.precision = "autocast"
        self.realesrgan_dir = "E:\stable-diffusion-webui-master\src\\realesrgan"
        ...
        self.no_job_manage = True
        self.max_jobs = 1
        self.optimizedSd_config = "E:\stable-diffusion-webui-master\optimizedSD/v1-inference.yaml"
...
class KDiffusionSampler:
    ...
    def sample(self, S, conditioning, batch_size, shape, verbose, unconditional_guidance_scale,
               unconditional_conditioning, eta, x_T, callback=None):
        ...
        samples_ddim = K.sampling.__dict__[f'sample_{self.schedule}'](model_wrap_cfg, x, sigmas,
                                                                      ...
                                                                      disable=False, callback=callback)
        print(samples_ddim)
...
def load_SD_model():
    if opt.optimized:
        ...
        config = OmegaConf.load(opt.optimizedSd_config)
...
def process_images(
    ...
    sample_path = os.path.join(outpath, prompt[:20]).strip()
...
def txt2img(prompt: str, ddim_steps: int = 35, sampler_name: str = "k_euler_a", toggles=None,
            realesrgan_model_name: str = opt.realesrgan_model,
            ddim_eta: float = 0.0, n_iter: int = 1, batch_size: int = 1, cfg_scale: float = 7.5,
            seed: Union[int, str, None] = '',
            height: int = 512, width: int = 512, fp=None, variant_amount: float = 0.0, variant_seed: int = None,
            job_info: JobInfo = None, percent_counter=None, out_dir_ff=None, callback=None):
    if toggles is None:
        toggles = [1, 2, 3]
...
def tensor_to_image(tensor):
    if opt.optimized:
        modelFS.to(device)
    sample = (model if not opt.optimized else modelFS).decode_first_stage(tensor)
    sample = torch.clamp((sample + 1.0) / 2.0, min=0.0, max=1.0)
    for elem in sample:
        elem = 255. * rearrange(elem.cpu().numpy(), 'c h w -> h w c')
        elem = elem.astype(np.uint8)
        return Image.fromarray(elem)
